...
import { fmtRs } from "@/lib/data";

export default function Services() {
  const { requests, reqAction, farms, syncAgriverse } = useFams();
  ...
  const open = requests.filter((r) => ["pending", "scheduled", "in_progress"].includes(r.state));
  const done = requests.filter((r) => r.state === "completed");
  const earned = done.reduce((s, r) => s + (r.total != null ? r.total : r.basePrice), 0);
  const pipeline = open.reduce((s, r) => s + (r.total != null ? r.total : r.basePrice), 0);
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
            <button 
              ...
              onClick={() => syncAgriverse()}
              style={{ background: "var(--surface-5)", height: 36 }}
            >
              Refresh
            </button>
