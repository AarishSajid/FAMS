...
export default function Advisories() {
  const { advisories, farms, broadcasts } = useFams();
  ...
  const rows = useMemo(() => {
    ...
    return list;
  }, [advisories, farms, q, stateFilter, typeFilter, sort]);
        ...
        <div className="card" style={{ padding: 12 }}>
          <div style={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={() => setShowGeneral(!showGeneral)}>
            <div className="card-title" style={{ marginBottom: 0 }}>
              General advisories today ({broadcasts.length}) — broadcast to all farms
            </div>
