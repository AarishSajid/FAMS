...
import {
  farms, farmById, generalAdvisories, earningsTrend, cycleBars,
  CYCLE, TODAY, fmtRs, farmWeather, farmStatus, STATUS_COLORS,
} from "@/lib/data";
...
export default function Dashboard() {
  ...
  const openReqs = requests.filter((r) => ["received", "scheduled", "in_progress"].includes(r.state));
  ...
  const pinFarms = useMemo(() => {
    ...
    if (view === "general")
      return farms.map((f) => ({ ...f, pinColor: "#2dd4bf", tipSub: `${generalAdvisories.length} general advisories today` }));
    return farms.map((f) => {
  ...
  const queue = useMemo(() => {
    const items = [
      ...advisories
        ...
        .map((a) => ({
          id: a.id, color: "var(--warning)",
          farmer: farmById[a.farmId].farmer,
          sub: `Verify: ${a.issueType} · due day 4 of cycle`,
          to: `/advisories/${a.farmId}`,
        })),
      ...openReqs.map((r) => ({
        id: r.id, color: "var(--info)",
        farmer: r.farmer,
      ...
      <main style={{ padding: 20, maxWidth: 1280, margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", gap: 14 }}>
          ...
          <Tile
            ...
            value={farmAdvToday + generalAdvisories.length}
            icon={ICONS.advisories}
            iconColor="var(--warning)"
            iconBg="var(--warning-muted)"
            sub={<>{farmAdvToday} farm-level · {generalAdvisories.length} general · <span style={{ color: "var(--warning)" }}>{toVerify} to verify</span> · <span style={{ color: "var(--danger)" }}>{pending} pending</span></>}
            onClick={() => router.push("/advisories")}
          />
          <Tile
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 14 }}>
          ...
          <div className="card" style={{ display: "flex", flexDirection: "column", height: 400, overflow: "hidden" }}>
            ...
            {!selFarmId && view === "general" && (
              <>
                <div className="card-title">General advisories today — {generalAdvisories.length}</div>
                <div style={{ fontSize: 11.5, color: "var(--text-muted)", marginBottom: 8 }}>
